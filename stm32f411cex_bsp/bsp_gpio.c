...
//1 or 0
uint8_t bsp_gpio_user_key_get_state(){
...
void bsp_gpio_PA7_init(void){
	...
	MODIFY_REG(GPIOA->OSPEEDR,GPIO_OSPEEDR_OSPEED7_Msk,GPIO_OSPEEDR_MS<<GPIO_OSPEEDR_OSPEED7_Pos);
	MODIFY_REG(GPIOA->PUPDR,GPIO_PUPDR_PUPD7_Msk,GPIO_PUPDR_PD<<GPIO_PUPDR_PUPD7_Pos);
}
...
void bsp_gpio_PA6_init(void){
	...
	MODIFY_REG(GPIOA->MODER,GPIO_MODER_MODE6_Msk,GPIO_MODER_INPUT<<GPIO_MODER_MODE6_Pos);
	MODIFY_REG(GPIOA->PUPDR,GPIO_PUPDR_PUPD6_Msk,GPIO_PUPDR_PU<<GPIO_PUPDR_PUPD6_Pos);
}
